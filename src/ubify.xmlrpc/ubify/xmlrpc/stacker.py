...
class StackerView(BrowserView):
    ...
    def setUserInfo(self,userdata=None):
        """
	Set all supplied name:value pairs in userdata dict on the user, provided that:
	1. The userdata *is* supplied (userdata should be a non-empty dict/struct), the function will throw
	a ValueError fault, if the userdata dict is empty
	2. The userdata will only be set for the currently logged in user
	"""
	if userdata is None or len(userdata)==0:
	    raise ValueError("userdata must be supplied!")
	userid = self.current_user()
    ...
    def getAllUserProperties(self):
        """
	Returns all available properties for current user
	"""
	pm = self.context.portal_membership
	user = pm.getAuthenticatedMember()
	if user is not None:
	    ps = user.getPropertysheet('mutable_properties')
	    return dict(ps.propertyItems())
	else:
	    raise ValueError("User %s Does not Exist!" % userid)
    ...
    def addNewComment(self,uid,comment_title,comment_body,reply_commentid=''):
        """Adds a comment on the provided UID with given title and text.
	The comment will be added with commentator as the user who is making this call.
