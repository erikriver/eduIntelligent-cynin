...
def triggerAddOnDiscussionItem(item):
    ...
    notifyobjectinitialization(item)
...
@forever.memoize
def getRootPhysicalPath():
    siteroot = getSite()
    rootid = contentroot_details['id']
    rootobj = getattr(siteroot,rootid,None)
    rootpath = '/'.join(rootobj.getPhysicalPath())
    return rootpath

@memoize_diy_request(arg=0)
def getLastChangeUID(request):
    ct = getToolByName(getSite(),'portal_catalog')
    query = {}
    query['portal_type'] = spacesdefaultaddablenonfolderishtypes + ('StatuslogItem',)
    query['sort_order'] = 'reverse'
    query['sort_on'] = 'cmf_uid'
    query['sort_limit'] = 1
    res = ct(query)
    if res is not None and len(res) > 0:
        ...
        #We got no results! No items have been added yet, or portal catalog is cleared.
        return None
...
def getLastChangeDate():
    ...
    res = ct(query)
    if res is not None and len(res) > 0:
        return res[0].lastchangedate
    else:
        #We got no results! No items have been added yet, or portal catalog is cleared.
        return None

@memoize_diy_request(arg=0)
def getLastChangeTimeElapsed(request):
    dt = getLastChangeDate()
    #print dt
    if dt is not None:
        return dt.millis()
...
def isLicensingInstalled():
    ...
    if licProd == True and eeProd == True:
        return True
    else:
        return False
...
def many_users():
    ...
    hasmanyusers = False
    if site_properties.hasProperty('many_users') and site_properties.many_users == True:
        hasmanyusers = True
    return hasmanyusers
