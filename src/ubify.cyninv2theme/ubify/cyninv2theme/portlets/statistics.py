...
from ubify.cyninv2theme import GetPortraitUrl
from ubify.cyninv2theme import getRootPhysicalPath
from itertools import chain
from ubify.cyninv2theme import many_users
...
def getRecentlyActiveMembers(context,path=None,depth=None,limitto=20):
    ct = getToolByName(context,'portal_catalog')
    lstinfo = []
    query = {}
    if path is not None and path != '':
    ...
    searchresults = ct(query)
    if len(searchresults)> 0:
        uniqueusers = unique([br.lastchangeperformer for br in searchresults])
        lstinfo = [{'userid':uu,'user_portrait_url':GetPortraitUrl(uu)} for uu in uniqueusers[:limitto]]
        return lstinfo
    else:
        return []

def listflatten(l, ltypes=(list, tuple)):
    ltype = type(l)
    l = list(l)
    i = 0
    while i < len(l):
...
def getMostUsedTags(context,path=None,depth=None,limitto=20):
    ct = getToolByName(context,'portal_catalog')
    try:
        #### XXX: This uses undocumented/private methods in the ZCatalog to return results, but it's waaaay FASTer. :)
        ki = ct._catalog.getIndex('Subject')
        tags = ki.uniqueValues(withLengths=True)
        if getRootPhysicalPath() != '/'.join(context.getPhysicalPath()):
            query = {}
            pathSubjects = []
            if path is not None and path != '':
                #get all tags of all items of context path
                query['path'] = {'query':path}
                query['portal_type'] = spacesdefaultaddablenonfolderishtypes + ('StatuslogItem',)
                pathSubjects = listflatten([item.Subject for item in ct(query)]) #Get a nice, flattened list of all tags in all items in path for context
                if len(pathSubjects) > 0:
                    pathSubjects = unique(pathSubjects)#Strip out duplicates in path tags
                    tags = [tag for tag in tags if tag[0] in pathSubjects] #Remove all tags not present in current context path
                else:
                    return []
        tags = sorted(tags,key=lambda occ_count: occ_count[1],reverse=True)
        lstinfo = [{'tagname':br[0],'count':br[1]} for br in (tags[:limitto])]
        return lstinfo
    except Exception, e:
        return []
